...
spatial_scanUp <- function(file_cas, file_pop, file_geo, date_1, date_2, scan_area, cluster_sz) {
  ...
  m <- leaflet(cluster_data) %>%
    ...
    addCircles(
      ...
      weight = 1,
      fillOpacity = 0.2,
      popup = ~paste("<b>CLUSTER DETAILS </b>", "<br>",
                     "<b>Cluster: </b>", CLUSTER, "<br>",
                     "<b>Analyzed period: </b>", START_DATE, END_DATE, "<br>",
                     "<b>Population: </b>", POPULATION, "<br>",
                     "<b>Number of cases: </b>", OBSERVED, "<br>",
                     "<b>Number of expected cases: </b>", EXPECTED, "<br>",
                     "<b>Number of cases/expected cases: </b>", ODE, "<br>",
                     "<b>Relative Risk: </b>", REL_RISK, "<br>",
                     "<b>P-Value: </b>" , P_VALUE
      ),
      group = "Detected clusters") %>%
    addCircleMarkers(
      data = location_data,
      ~LOC_Y,
      ~LOC_X,
      ...
      group = "Identified locations") %>%
    addLayersControl(
      baseGroups = c("Map"),
      overlayGroups = c("Detected clusters", "Identified locations"),
      options = layersControlOptions(collapsed = FALSE)
    ) %>%
    addLegend(position = "bottomright",
              colors = c("red", "blue"),
              labels = c("Significant (p < 0.05)" , "Not significant (p > 0.05)"),
              title = "cluster statistical significance",
              opacity = 0.4) %>%
